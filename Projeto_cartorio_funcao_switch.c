...
int excluir()
{
	...
	if (file == NULL)
	{
		printf("\nCPF que deseja excluir não foi localizado na base de dados.\n");
		system("pause");
		return 0;
	}
	
	printf("Deseja mesmo excluir o seguinte usuário do sistema:");
	printf("%s", cpf);
	printf("\n\nDigite (s) para confirmar e prosseguir com a exclusão ou (n) para voltar ao menu:\n\n");
	system("pause");
	
	getchar(); //funçaõ para capturar a entrada do usuário
	
	char opcao;
	opcao = getchar(); // definição que a variavel opcao será referente a getchar
	
	switch(opcao) //usando o switch case para a partir das variáveis declaradas pelo usuário, determinar a exclusão ou não;
	{
	
		case 's':
		remove(cpf);
		system("pause");
		printf("\n O usuário foi excluído da base de dados!");
		system("pause");
		break;
		...
		case 'n':
		printf("Você será redirecionado ao menu.");
		system("pause");
		return 0;
		...
		default:
		printf("A opção desejada não está disponível. Escolha (s) ou (n)");	
		system("pause");
		break;
	}
}
